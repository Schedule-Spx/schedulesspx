            ...
            {Object.entries(themes).map(([themeName, theme]) => (
              <div key={themeName} className="flex flex-col items-center">
                ...
                <button
                  ...
                  className={`mt-2 ${currentTheme.main} ${currentTheme.text} font-bold py-2 px-4 rounded hover:opacity-80 transition-opacity duration-200 w-full ${currentTheme.name === themeName ? `ring-2 ${currentTheme.border}` : ''}`}
                >
                  {theme.name}
                </button>
              </div>
