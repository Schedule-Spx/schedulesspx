...
const Admin = ({ weekSchedule, setWeekSchedule, fetchSchedule }) => {
    ...
    // Modify the savePopup function in Admin.jsx to include the real author information
    ...
    const savePopup = async () => {
        try {
            ...
            // Get the current user's display name from Google auth
            const realAuthor = user ? user.displayName || user.email : 'Unknown Admin';

            // Include the realAuthor field in the payload
            const popupWithRealAuthor = {
                ...popup,
                realAuthor // This field will be stored but not shown to public
            };
            ...
            const response = await fetch('https://schedule-api.devs4u.workers.dev/api/popup', {
                ...
                body: JSON.stringify(popupWithRealAuthor)
            });
                    ...
                    <div className={`${currentTheme.secondary} border-2 ${currentTheme.border} p-6 rounded-lg shadow-lg`}>
                        ...
                        <div className="mb-8">
                            ...
                            <div className="mb-4">
                                ...
                                {popup.realAuthor && (
                                    <p>Posted by: {popup.realAuthor}</p>
                                )}
                            </div>
                            <div className="mb-4">
